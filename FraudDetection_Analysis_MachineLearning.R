...
data_pca_merge$Var6 = ny_property_merge$AVTOT_LOTAREA / ny_property_merge$ZIP_AVTOT_LOTAREA_MEAN
...
data_pca_merge$Var15 = ny_property_merge$AVTOT_LOTAREA / ny_property_merge$ZIP3_AVTOT_LOTAREA_MEAN
...
data_pca_merge$Var24 = ny_property_merge$AVTOT_LOTAREA / ny_property_merge$TAXCLASS_AVTOT_LOTAREA_MEAN
...
#compute fraud score 11 from prcomp function (from stats package and h2o library package)
...
#Updated february 12,2018. Computing fraud_score 111 from principal function ( psych library package)
...
View(fraud_score11_autoencoder)
...
View(autoencoder_fraudscore11)

#rescaling it for range 0 to 1
autoencoder_fraudscore22 <- (autoencoder_fraudscore - min(autoencoder_fraudscore11)) / (max(autoencoder_fraudscore11)  - min(autoencoder_fraudscore11))

min(autoencoder_fraudscore11)
View(autoencoder_fraudscore22)
...
biplot(data_pca_result, scale = 0)


#Overlap looking at the top 1% (10,000)

outlierdetection_fraudscore11_top10000 <- outlierdetection_fraudscore11 %>%
  arrange(-fraud_score11) %>%
  slice(1:10000)

autoencoder_fraudscore2_top10000 <- autoencoder_fraudscore2_dataframe %>%
  arrange(-autoencoder_fraudscore) %>%
  slice(1:10000)

#lets do an inner join to return the matching values 
overlap_data1 <- inner_join(outlierdetection_fraudscore11_top10000, autoencoder_fraudscore2_top10000, by = 'record_id')

#Matching Percentage

# overlap_data1 has 8529 obersations so matching percentage is about 85.29%



# Professor just sent an email feburary 18th
# Now, for all the records in the highest bin, replace the score by the number 1000. So the top, highest-scoring .1% of the records now all have the same score, 1000.
# For the next bin, which is the next .1% highest scoring records, replace their score by the number 999. The second-highest .1% scoring records now all have the same score of 999.
# Keep going, replacing the score value on the records by the bin number of where they are in the sorted order.

# rescaling our score from 0 to 1000 of the top 1% of the data of outlier detection fraud score
# Note that the dataset is in descending order already



outlierdetection_fraudscore11 <- outlierdetection_fraudscore11 %>%
  arrange(-fraud_score11)



for (i in 1:(nrow(outlierdetection_fraudscore11)*0.01)) {
  outlierdetection_fraudscore11$newscore[i] = 1000
  for (j in (nrow(outlierdetection_fraudscore11)*0.01) : (nrow(outlierdetection_fraudscore11)*0.02)) {
    outlierdetection_fraudscore11$newscore[j] = 999
    for (k in (nrow(outlierdetection_fraudscore11)*0.02) : (nrow(outlierdetection_fraudscore11)*0.03)) {
      outlierdetection_fraudscore11$newscore[k] = 998
    }
  }
} #Nope not working too long 

h = hist(outlierdetection_fraudscore11$fraud_score11, breaks=(min(x)-1):(max(x))+0.5)
...
freq <- hist(outlierdetection_fraudscore11$fraud_score11, breaks=1000 , plot = F )

freq$counts



bou <- data.frame(outlierdetection_fraudscore11$fraud_score11, bin=cut(outlierdetection_fraudscore11$fraud_score11, 0:1000, include.lowest=TRUE))




#Visualizing the zeros of the two fraud_score


ggplot(data = autoencoder_fraudscore2, aes(x = autoencoder_fraudscore2)) +
  geom_histogram(bins = 100, fill = "dodgerblue3" , col = "black") +
  ggtitle("Autoencoder Fraud Score") +
  xlab("Fraud Score") +
  ylab("count") +
  theme(plot.title = element_text(size = 20, hjust = 0.5),
        axis.title.x = element_text(size = 18),
...
ggplot(data = fraud_score11_autoencoder, aes(x = fraud_score11_autoencoder)) +
  geom_histogram(bins = 100, fill = "dodgerblue3" , col = "black") +
  scale_y_log10() +
  ggtitle("Outlier detection via z-scores: Heuristic Fraud Score") +
  xlab("Fraud Score") +
  ylab("count") +
  theme(plot.title = element_text(size = 20, hjust = 0.5),
        axis.title.x = element_text(size = 18),
...
g <- ggplot(data = outlierdetection_fraudscore11, aes(x = fraud_score11)) +
  geom_histogram(bins = 100, fill = "dodgerblue3" , col = "black") +
  ggtitle("Outlier detection via z-scores: Fraud Score of Autoencoder") +
  xlab("Fraud Score") +
  ylab("count") +
...
summary(autoencoder_fraudscore2222)
pg <- ggplot_build(g)
...
scorebin <- ntile(autoencoder_fraudscore22$autoencoder_fraudscore,1000)

length(unique(scorebin))

#Function in dplyr for quantile bining 
#heuristic 

fraud_score11_heuristic <- fraud_score11_autoencoder

fraud_score11_heuristic$scorebin <- ntile(fraud_score11_heuristic$fraud_score11,1000)

fraud_score11_heuristic %>%
  filter(scorebin == 1000 )%>%
  arrange(desc(fraud_score11))
...
autoencoder_fraudscore22$scorebin <- ntile(autoencoder_fraudscore22$autoencoder_fraudscore,1000)
View(autoencoder_fraudscore22)

autoencoder_fraudscore22 %>%
  filter(scorebin == 998 )%>%
  arrange(desc(autoencoder_fraudscore))


#comparing thr bins 1000 of both score

fraud_score11_heuristic_1000 <- fraud_score11_heuristic %>%
  filter(scorebin == 1000) 

autoencoder_fraudscore22_1000 <- autoencoder_fraudscore22 %>%
  filter(scorebin == 1000)

?quantile
str(fraud_score11_heuristic)

fraud_score11_heuristic$scorebin <- as.integer(fraud_score11_heuristic$scorebin)


#Looking at the summary stats of heuristic by analyzing the bins
heuristic_bindata <- fraud_score11_heuristic %>%
  group_by(scorebin) %>%
  summarise( Min = min(fraud_score11),
             Q1 = quantile(fraud_score11,0.25),
             Mean = mean(fraud_score11),
             Median = median(fraud_score11),
             Q3 = quantile(fraud_score11,0.75),
             Max = max(fraud_score11),
             SD = sd(fraud_score11)) %>%
  arrange(desc(scorebin))

View(heuristic_bindata)

#Looking at the summary stats of autoencoder by analyzing the bins

autoencoder_bindata <- autoencoder_fraudscore22 %>%
  group_by(scorebin) %>%
  summarise( Min = min(autoencoder_fraudscore),
             Q1 = quantile(autoencoder_fraudscore,0.25),
             Mean = mean(autoencoder_fraudscore),
             Median = median(autoencoder_fraudscore),
             Q3 = quantile(autoencoder_fraudscore,0.75),
             Max = max(autoencoder_fraudscore),
             SD = sd(autoencoder_fraudscore)) %>%
  arrange(desc(scorebin))

View(autoencoder_bindata)

write.csv(heuristic_bindata, "heuristic_bindata.csv")
write.csv(autoencoder_bindata, "autoencoder_bindata.csv")
